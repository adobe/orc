...
enum class at : std::uint16_t {
    ...
    accessibility = 0x32,
    address_class = 0x33,
    // DW_AT_artificial attribute indicates that the associated entity (e.g., a function, variable,
    // or parameter) is compiler-generated rather than explicitly written by the programmer in the
    // source code.
    artificial = 0x34,
    base_types = 0x35,
    ...
    decl_file = 0x3a,
    decl_line = 0x3b,
    // DW_AT_declaration indicates that the associated entity is a declaration rather than a
    // definition. A function declaration is typically represented as a DW_TAG_subprogram entry with
    // the attribute DW_AT_declaration set to true (or 1). It does not have attributes like
    // DW_AT_low_pc or DW_AT_high_pc, as it does not correspond to actual code. Example:
    // <1><0x0000003a>    DW_TAG_subprogram
    //                     DW_AT_name                  ("myFunction")
    //                     DW_AT_declaration           (true)

    // A function implementation is also represented as a DW_TAG_subprogram entry but does not have
    // the DW_AT_declaration attribute. Instead, it includes attributes like DW_AT_low_pc and
    // DW_AT_high_pc (or DW_AT_ranges), which specify the address range of the function's code in
    // memory. Example: <1><0x0000003a>    DW_TAG_subprogram
    //                     DW_AT_name                  ("myFunction")
    ...
    discr_list = 0x3d,
    encoding = 0x3e,
    // DW_AT_external attribute indicates that the corresponding entity (e.g., a variable, function,
    // or type) has external linkage.
    external = 0x3f,
    frame_base = 0x40,
    friend_ = 0x41,
