...
/*
    ...
    * Two `pool_strings` pointing to the same `_data` are always equal, and if the `_data` is
   different, they are not equal.<
    * `_data` is a char* to null terminated data, which is just a c string. Useful for debugging.
    * if `_data` is null, it is intepreted as an empty string (""). If `_data` is not-null, it
   always is size() > 0

    When empooled, the hash (64 bits) and size/length (32 bits) are stored before the char* to the
   data. Note there is no memory alignment in the pool - it is fully packed - so data needs to be
   memcpy'd in and out, just in case the processor doesn't like un-aligned reads.
*/
struct pool_string {
    pool_string() = default;
    ~pool_string() = default;
    ...
    std::string allocate_string() const { return std::string(view()); }

    std::filesystem::path allocate_path() const { return std::filesystem::path(view()); }
    ...
    friend inline bool operator!=(const pool_string& x, const pool_string& y) { return !(x == y); }
...
// of ORC's considerable memory usage. pool_string doesn't have a copy constructor or move
// semantics. Copying and low memory usage depend on pool_string being really a pointer, so double
// check that here, and don't remove this unless you are careful about performance of large
// projects.
static_assert(sizeof(pool_string) <= sizeof(intptr_t),
              "pool_string is design to be as small and fast to copy as a pointer.");
