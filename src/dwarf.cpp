...
struct dwarf::implementation {
    ...
    // Called in between on-disk DWARF section discovery and die processing
    // to set up the state machine to process dies.
    void finalize_section_registration();
...
void dwarf::implementation::finalize_section_registration() {
    if (_ready) {
        return;
    }

    // Houston, we have a problem.
    if (!_debug_info.valid()) {
        throw std::runtime_error("Unread section: debug_info");
    }
    if (!_debug_abbrev.valid()) {
        throw std::runtime_error("Unread section: debug_abbrev");
    }
    if (!_debug_line.valid()) {
        throw std::runtime_error("Unread section: debug_line");
    }
